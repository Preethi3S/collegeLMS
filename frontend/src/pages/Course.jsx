...
const CoursePage = () => {
  ...
  const isModuleCompleted = (moduleId) =>
    progress?.levels?.some((lvl) => lvl.moduleProgress.some((m) => m.moduleId === moduleId && m.completed));
  ...
  const getCompletedModules = () => {
    ...
    progress?.levels?.forEach((lvl) => {
      count += lvl.moduleProgress.filter((m) => m.completed).length;
    });
    return count;
  };
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div>
                        ...
                        <Typography variant="body2" fontWeight={600}>
                          {Math.round(
                            progress.levels.flatMap((l) => l.moduleProgress)
                              .find((m) => m.moduleId === selectedModule._id)?.percentWatched || 0
                          )}% watched
                        </Typography>
                      </div>
