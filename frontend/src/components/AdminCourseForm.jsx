...
const AdminCourseForm = ({
  ...
  const addCodingQuestion = (levelIndex, moduleIndex) => {
    const updated = [...levels];
  ...
  const removeCodingQuestion = (levelIndex, moduleIndex, qIndex) => {
    const updated = [...levels];
    if (updated[levelIndex].modules[moduleIndex].codingQuestions) {
  ...
  const updateCodingQuestion = (levelIndex, moduleIndex, qIndex, field, value) => {
    const updated = [...levels];
    if (updated[levelIndex].modules[moduleIndex].codingQuestions) {
        ...
        {levels.map((level, li) => (
          <Paper key={li} sx={{ p: 2, mb: 2 }}>
            ...
            {level.modules.map((mod, mi) => (
              <Box key={mi} sx={{ ml: 2, mb: 1, borderLeft: '2px solid #ddd', pl: 2 }}>
                <TextField
                  fullWidth
                  label="Module Title"
                  sx={{ my: 0.5 }}
                ...
                {mod.codingQuestions && mod.codingQuestions.map((q, qi) => (
                  <Box key={qi} display="flex" gap={1} mb={1}>
                    <TextField
                      size="small"
                      placeholder="Title"
                      value={q.title}
                    ...
                    />
                    <IconButton size="small" onClick={() => removeCodingQuestion(li, mi, qi)}>
                      <Delete fontSize="small" />
                    </IconButton>
                  </Box>
                ))}
                <Button size="small" onClick={() => addCodingQuestion(li, mi)}>
                  + Add Coding Question
                </Button>
              </Box>
            ))}
            <Button size="small" startIcon={<AddCircle />} onClick={() => addModule(li)}>
              Add Module
            </Button>
          </Paper>
        ))}

        <Button startIcon={<AddCircle />} onClick={addLevel} sx={{ mt: 1 }}>
          Add Level
        </Button>
      </DialogContent>
      <DialogActions>
